...
function() {
	"use strict";
	var t = {
		getActivityList: "/cms/notice/superred",
		template: function(t) {
			console.log(t);
			var i = function() {
			...
			return '<div class="activity_popup" ><div class="center_main" style="' + ($(window).width() > 750 ? "width:550px" : "") + '">' + (i() ? '<span class="code">' + t.code + "</span>" : '<a target="_blank" href="' + cms_config.base_url+t.url + '" class="link">&nbsp;</a>') + '<a href="javascript:;" class="activity_close"></a><img src="' + (i() ? t.window_tkl_img : t.window_img) + '" alt="" class="bg" /> ' + (i() ? '<div class="copy"><a class="copyCode"  aria-label=" ' + t.kouling + ' " >一键复制</a></div>' : "") + '<div class="copy-ell"><p>复制失败</p>请长按淘口令手动复制</div></div></div><div class="activity_popup_bg"></div>'
		},
		getHost: function() {
			var t = "";
	...
	i.prototype.init = function() {
		...
		this.getActivityItem(function(o) {
			return 0 != parseInt(o.is_on) && ($("body").append(t.getStyleItem()).append('<a class="tmall_home_tab"><img src="' + o.window_right_img + '" alt=""></a>'), $(".tmall_home_tab").on("click", function(i) {
				i.preventDefault(), e() ? t.delCookie("ACTIVITY_POPUP") : window.location.href = cms_config.base_url+o.url
		...
		}, 50), t.getJavaScript("/wap_new/lib/clipboard.min.js", function() {
			setTimeout(function() {
				e.copy(a)
			}, 500)
		})
		...
		var e = this;
		$.ajax({
			url: cms_config.s+t.getActivityList,
			type: "post",
			dataType: "JSON",
			data: {
				a: 1
			...
			0 == t.info.status && t.data && t.data.item && (e.item = t.data.item, i(t.data.item))
		})
