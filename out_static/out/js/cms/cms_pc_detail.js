...
$(function() {
	$(".check_img_show").length <= 0 && $(".goods-detail").hide();
	var t = $("#jp_container_1").data("video"),
		a = $("#jp_container_1").data("post");
	...
	n.height(s * l), $(".bot-btn").on("click", function() {
		...
		t.hasClass("disabled") || n.is(":animated") || n.animate({
			top: a + 3 * s
		}, c, function() {
...
$(function() {
	...
	} else if (2 == activity.type || 3 == activity.type || 5 == activity.type) $(".active-type .last-time.tjd").show(), a(activity.goods_time, ".last-time.tjd");
	else if (4 == activity.type) $(".active-type .rank").show(), $(".rank .rank-num i").text(activity.sales_num), $(".rank .rank-list b").text(activity.rownum);
	else if (6 == activity.type) {
	...
	$(".pointer").eq(0).addClass("active"), $(".pagination .prev").on("click", function() {
		...
		effect: "fadeIn"
	}), 0 == $(".sm-img li").length && $(".act-bg").css("width", "584px")
}), !
function() {
	...
	t.prototype.init = function() {
		...
		var a = this,
			e = $(".detail_layer_image");
		e.find(".tab a").unbind("click").on("click", function(t) {
			t.preventDefault();
			var i = this,
				n = function() {
					e.find(".tab a").removeClass("active"), $(i).addClass("active"), e.find(".items>div").hide(), e.find(".items>div:eq(" + $(i).index() + ")").show(), layer.style(a.openLayerIndex, {
						height: 0 === $(i).index() ? "700px" : "450px"
					})
				};
			if (1 != $(this).index() || a.openUrl) n();
			else {
				var o = layer.load();
				a.ajax({
					url: "?r=l/shorturl",
		...
		var n = window.navigator.userAgent.toLowerCase(),
			o = "function" == typeof Promise && n.indexOf("mac") < 0;
