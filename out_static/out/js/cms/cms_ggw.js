...
$(document).ready(function(e) {
			...
			if (0 == e.status && e.data.length > 0) {
				if (e.data[0].cms_web_index && e.data[0].cms_web_index.data.length > 0) {
					var t = e.data[0].cms_web_index,
						...
						i = 1 == s.open_type ? "_blank" : "_self",
						c = "{desc:'弹窗入口流量表'}",
						n = "{desc:'弹窗入口展示',name:'PopupWindowsShow'}";
					try {
						DTKsatc.eventStat({
							name: "PopupWindowsShow",
							params: {
								desc: "弹窗入口展示"
							}
						})
	...
	var o = function() {
			if (t++, t < 200) if (0 == $(".img_size img").width()) setTimeout(arguments.callee, 10);
			else {
				var e = $(".img_size img").width(),
					a = $(".img_size img").height();
