...
<?
    ...
    if(isset($arr_query['tmall'])){
        if(strstr($base_url,"?")){$base_url = $base_url.'&';}else{$base_url = $base_url.'?';}
        $base_url = $base_url.'tmall='.$arr_query['tmall'];
    }
    if(!empty($arr_query['volume'])){
        if(strstr($base_url,"?")){$base_url = $base_url.'&';}else{$base_url = $base_url.'?';}
        $base_url = $base_url.'volume='.$arr_query['volume'];
    }
    if(!empty($arr_query['price1'])){
        if(strstr($base_url,"?")){$base_url = $base_url.'&';}else{$base_url = $base_url.'?';}
        $base_url = $base_url.'price1='.$arr_query['price1'];
    }
    if(!empty($arr_query['price2'])){
        if(strstr($base_url,"?")){$base_url = $base_url.'&';}else{$base_url = $base_url.'?';}
        $base_url = $base_url.'price2='.$arr_query['price2'];
    }
    ...
    if(!empty($arr_query['tmall'])){
        if(strstr($cate_url,"?")){$cate_url = $cate_url.'&';}else{$cate_url = $cate_url.'?';}
        $cate_url = $cate_url.'tmall='.$arr_query['tmall'];
    }
    if(strstr($cate_url,"?")){$cate_url = $cate_url.'&';}else{$cate_url = $cate_url.'?';}

    $type_url = $base_url;
    if(!empty($arr_query['sort'])){
        if(strstr($type_url,"?")){$type_url = $type_url.'&';}else{$type_url = $type_url.'?';}
        $type_url = $type_url.'sort='.$arr_query['sort'];
    }
    if(!empty($arr_query['order_next'])){
        if(strstr($type_url,"?")){$type_url = $type_url.'&';}else{$type_url = $type_url.'?';}
        $type_url = $type_url.'order_next='.$arr_query['order_next'];
    }
    if(!empty($arr_query['cid'])){
        if(strstr($type_url,"?")){$type_url = $type_url.'&';}else{$type_url = $type_url.'?';}
        $type_url = $type_url.'cid='.$arr_query['cid'];
    }
    if(strstr($type_url,"?")){$type_url = $type_url.'&';}else{$type_url = $type_url.'?';}
?>
<div id="filter">
...
<div class="col-xs-12">
<ul class="filter-nav">
<li class="<?php if(empty($arr_query['cid'])){ ?>cur<?php } ?>"><a class="cnzzCounter" data-cnzz-type="51" data-cnzz="100" href="<?php echo $cate_url; ?>cid=">全部（<?php echo $goods_count; ?>）</a></li>
<?php
...
<input type="hidden" name="action" value="temai"  />
...
<div id="yw0" class="pager">
        ...
        if(!empty($arr_query['tmall'])){
            if(strstr($pager_url,"?")){$pager_url = $pager_url.'&';}else{$pager_url = $pager_url.'?';}
            $pager_url = $pager_url.'tmall='.$arr_query['tmall'];
        }
        if(!empty($arr_query['sort'])){
            if(strstr($pager_url,"?")){$pager_url = $pager_url.'&';}else{$pager_url = $pager_url.'?';}
